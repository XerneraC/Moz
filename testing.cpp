...
namespace ComplexTests {
	TEST_CASE("Complex initialization", "[complex]") {
		fcomp n1 = fcomp(2.f, 3.f);
		REQUIRE(n1.real == 2.f);
		REQUIRE(n1.imag == 3.f);

		fcomp n2 = fcomp(1.f, 7.f);
		REQUIRE(n2.x == 1.f);
		REQUIRE(n2.y == 7.f);
	...
	TEST_CASE("Automatic implicit conversion", "[Complex]") {
		fcomp test = fcomp(1.f, 2.f) + 2.f;
		REQUIRE(test.real == 1.f + 2.f);
		REQUIRE(test.imag == 2.f);
	}

	// TODO: Add test for complex arithmetic (I'm too bored rn and I trust that it will be good)
}